...
**Business Rules:**
- BR-FLIGHT-01 — Required search fields: fromCity, toCity, startDate are required.
- BR-FLIGHT-02 — Date range filtering: Departure date filters flights within 24-hour window of the selected date.
- BR-FLIGHT-03 — Passenger count validation: Total passengers (adults + minors) must be <= available seats.
- BR-FLIGHT-04 — Round-trip logic: If type=true and endDate is provided, returning flights are queried with reversed cities.
...
**Business Rules:**
- BR-SELECT-01 — Single selection: Only one departing flight can be selected at a time.
- BR-SELECT-02 — Round-trip requirement: If trip type is round-trip, both departing and returning flights must be selected.
- BR-SELECT-03 — Selection persistence: Selected flights are stored in localStorage for state management.
...
**Business Rules:**
- BR-PASSENGER-01 — Required fields: firstName, lastName, dateOfBirth, email, phone, checkedBags are required.
- BR-PASSENGER-02 — Email validation: Email must match pattern /^[^\s@]+@[^\s@]+\.[^\s@]+$/.
- BR-PASSENGER-03 — Phone validation: Phone must be exactly 11 digits.
- BR-PASSENGER-04 — Date of birth validation: Date of birth must be less than current date.
- BR-PASSENGER-05 — Optional fields: middleName, suffix, redressNumber, knownTravelerNumber are optional.
...
**Business Rules:**
- BR-SEAT-01 — Seat availability: Only seats with available=true are displayed.
- BR-SEAT-02 — Seat types: Seats are categorized as Economy or Business.
- BR-SEAT-03 — Upgrade logic: Selecting Business seat from Economy triggers upgrade popup with price difference.
- BR-SEAT-04 — Round-trip seats: If round-trip, both departing and returning seats must be selected.
...
**Business Rules:**
- BR-PAYMENT-01 — Card validation: Card number must be 16 digits, CCV must be 3 digits, expiration date must be in future.
- BR-PAYMENT-02 — Payment type: Only Visa payment type is supported.
- BR-PAYMENT-03 — Guest booking: userId can be null for guest booking.
- BR-PAYMENT-04 — Price calculation: Total = subtotalPrice + taxesAndFees + baggageFees + upgradeFees.
- BR-PAYMENT-05 — Confirmation code: Confirmation code is generated as UUID (12 characters).
...
**Business Rules:**
- BR-CONFIRM-01 — Confirmation display: Confirmation code, flight summary, and price breakdown are displayed.
- BR-CONFIRM-02 — Share itinerary: Visitor can share itinerary by entering email address.
- BR-CONFIRM-03 — Hotel suggestions: Hotel suggestions are displayed for cross-selling.
...
**Business Rules:**
- BR-SIGNUP-01 — Email validation: Email must contain @ symbol.
- BR-SIGNUP-02 — Password length: Password must be at least 8 characters.
- BR-SIGNUP-03 — Terms agreement: Visitor must agree to terms and conditions.
- BR-SIGNUP-04 — Email uniqueness: Email must not already exist in database.
- BR-SIGNUP-05 — Username generation: Username is generated as {emailBase}_{timestamp}.
- BR-SIGNUP-06 — Password hashing: Password is hashed with bcrypt using 10 salt rounds.
- BR-SIGNUP-07 — Default location: New users are assigned city=Cairo and country=Egypt (hardcoded).
...
**Business Rules:**
- BR-SIGNIN-01 — Email validation: Email must contain @ symbol.
- BR-SIGNIN-02 — Password length: Password must be at least 8 characters.
- BR-SIGNIN-03 — Password verification: Password is verified using bcrypt compare.
- BR-SIGNIN-04 — Session strategy: JWT session strategy is used.
- BR-SIGNIN-05 — Google OAuth: Google OAuth provider is configured with env variables GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET.
...
**Business Rules:**
- BR-CITIES-01 — Distinct cities: Cities are extracted with distinct to remove duplicates.
- BR-CITIES-02 — City arrays: fromCities contains all unique departure cities, toCities contains all unique arrival cities.
...
**Business Rules:**
- BR-DEALS-01 — Deal display: Deals show image, price, and description.
- BR-DEALS-02 — Deal booking: Clicking deal pre-fills search parameters and redirects to flights page.
...
**Business Rules:**
- BR-PLACES-01 — Place display: Places show image, price, description, and motivation.
- BR-PLACES-02 — Place booking: Clicking place pre-fills search parameters and redirects to flights page.
...
**Business Rules:**
- BR-SHARE-01 — Email validation: Email must match pattern /^[^\s@]+@[^\s@]+\.[^\s@]+$/.
- BR-SHARE-02 — Share record: Share record links bookingId with recipient email.
- BR-SHARE-03 — Email sending: Itinerary email is sent to recipient.
...
**Business Rules:**
- BR-HISTORY-01 — User filtering: Bookings are filtered by userId.
- BR-HISTORY-02 — Booking details: Each booking includes flight details, passenger info, and payment info.
...
**Business Rules:**
- BR-CANCEL-01 — Booking status: Booking status is updated to cancelled.
- BR-CANCEL-02 — Refund policy: Refund is processed based on cancellation policy (not implemented).
- BR-CANCEL-03 — Cancellation deadline: Cancellation must be done before deadline (not implemented).
